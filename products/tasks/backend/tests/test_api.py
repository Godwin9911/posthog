...
class TestTaskRunAPI(BaseTaskAPITest):
    ...
    @parameterized.expand(
        [
            ("resync_capable_client", "?resync=true", "https://agent-proxy.example.com"),
            ("legacy_client", "", None),
        ]
    )
    @override_settings(SANDBOX_JWT_PRIVATE_KEY=TEST_RSA_PRIVATE_KEY)
    def test_stream_token_routes_thin_tail_run_by_client_resync_support(self, _name, query, expected_base_url):
        reset_sandbox_jwt_key_cache()

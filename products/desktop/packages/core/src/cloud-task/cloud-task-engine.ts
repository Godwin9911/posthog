...
export class CloudTaskEngine extends TypedEventEmitter<CloudTaskEvents> {
  ...
  private async bootstrapWatcher(
    ...
    watcher.bufferedLogBatches = [];
    void this.connectSse(key, {
      startLatest: !watcher.pendingStreamWindowReplay,
    });
    ...
    watcher.isBootstrapping = false;
    this.drainBufferedLogBatches(key, snapshotEntries);
  ...
  private drainBufferedLogBatches(
    ...
    const matcher = new HistoricalEntryMatcher(historicalEntries);

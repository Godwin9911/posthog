...
export class IngestionSessionReplayMlMirrorServer extends MlMirrorConsumerServer {
    ...
    protected async startServices(): Promise<void> {
        ...
        const collaborators: SessionRecordingIngesterCollaborators = {
            ...
            createStagedRunner: (pipelineConfig) => {
                const mlOptions = {
                    keyManager,
                ...
                return new MlMirrorStagedBatchRunner(
                    createMlMirrorPreparePipeline(pipelineConfig, mlOptions),
                    createMlMirrorAnonymizePipeline(
                        pipelineConfig,
                        mlOptions,
                        this.config.SESSION_RECORDING_ML_IMAGE_SCRUB_PRODUCER_ENABLED
                            ? {
                                  outputs,
                                  producedRefCacheMax:
                                      this.config.SESSION_RECORDING_ML_IMAGE_SCRUB_PRODUCED_REF_CACHE_MAX,
                              }
                            : undefined,
                        {
                            pseudonymSecret,
                    ...
                    pipelineConfig.promiseScheduler
                )
            },

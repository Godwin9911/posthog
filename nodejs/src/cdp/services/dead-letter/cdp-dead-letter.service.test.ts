...
import { HogFunctionInvocationGlobals, InvocationBuildFailure } from '../../types'
...
describe('CdpDeadLetterService', () => {
    ...
    it('parks the original bytes with the source position and the failed function', async () => {
        ...
        service.recordBuildFailures(defaultGlobals, [failure()])
    ...
    it('still parks a whole batch of filter and input failures', async () => {
        ...
        service.recordBuildFailures(defaultGlobals, [failure()])
        service.recordBuildFailures(second, [failure()])
    ...
    it('gives each team its own record when two events collide on the same client-supplied ID', async () => {
        ...
        service.recordBuildFailures(teamA, [failure({ sourceId: 'fn-a' })])
        service.recordBuildFailures(teamB, [failure({ sourceId: 'fn-b' })])
